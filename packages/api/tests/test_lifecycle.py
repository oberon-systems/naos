...
from naos_api.lifecycle import ACTIVE, TERMINAL, TRANSITIONS, RunStatus, ensure_transition
...
def test_transition_table_matches_lifecycle() -> None:
    assert {status: set(targets) for status, targets in TRANSITIONS.items()} == EXPECTED
