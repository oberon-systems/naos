...
def test_unknown_policy_reference_is_unprocessable(
    ...
    spec_body["shell"] = {"policy_snapshot": "shellpol_" + "0" * 32}
...
def test_mount_snapshot_flow(
    client: TestClient, spec_body: dict[str, Any], mount_body: dict[str, Any]
) -> None:
    body = {"kind": "mount", "document": mount_body}
    created = client.post("/api/v1/policy-snapshots", json=body)
    replayed = client.post("/api/v1/policy-snapshots", json=body)
    snapshot_id = created.json()["id"]

    assert created.status_code == 201
    assert replayed.status_code == 200
    assert replayed.json()["id"] == snapshot_id
    assert created.json()["document"]["workdir"] == "/naos/alpha"
    assert client.get(f"/api/v1/policy-snapshots/{snapshot_id}").json() == created.json()

    spec_body["mounts"] = {"policy_snapshot": snapshot_id}
    run = _create(client, spec_body).json()
    assert run["spec"]["mounts"]["policy_snapshot"] == snapshot_id
...
def test_bad_snapshot_is_unprocessable(client: TestClient, body: dict[str, Any]) -> None:
    assert client.post("/api/v1/policy-snapshots", json=body).status_code == 422


def test_unknown_snapshot_is_not_found(client: TestClient) -> None:
    assert client.get("/api/v1/policy-snapshots/mntpol_missing").status_code == 404
